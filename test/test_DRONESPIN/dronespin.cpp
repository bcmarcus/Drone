...
void headingHold() {
  error_heading = desired_heading - heading;
  P_yaw = Kp_yaw * error_heading;
}

void spinInPlace(float spin_speed) {
  P_yaw = Kp_yaw * spin_speed; // spin_speed is a positive or negative value depending on the desired spin direction
}

void applyHeadFreeMode(sensors_event_t &a) {
  float temp_x = a.acceleration.x;
  float temp_y = a.acceleration.y;
  float angle_rad = -error_heading * PI / 180.0;
  a.acceleration.x = temp_x * cos(angle_rad) - temp_y * sin(angle_rad);
  a.acceleration.y = temp_x * sin(angle_rad) + temp_y * cos(angle_rad);
}
...
void loop() {
  ...
  // Apply head-free mode if enabled
  if (head_free_mode) {
    applyHeadFreeMode(a);
  }
