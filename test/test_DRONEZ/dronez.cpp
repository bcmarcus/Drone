...
void setup() {
  ...
  gy87.setGyroOffsets(2.42, 0.05, -0.23);
...
void loop() {
  ...
  if (ready) {
    ...
    unsat_throttle[0] = current_motor_speed_0 + P_z + I_z + D_z;
    unsat_throttle[1] = current_motor_speed_1 - P_z - I_z - D_z;
    unsat_throttle[2] = current_motor_speed_2 - P_z - I_z - D_z; // Motor 3 (negative roll, negative pitch, clockwise)
    unsat_throttle[3] = current_motor_speed_3 + P_z + I_z + D_z; // Motor 4 (negative roll, positive pitch, counterclockwise)
