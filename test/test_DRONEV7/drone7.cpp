...
void setup() {
  ...
  gy87.setAccOffsets(0.05, 0.02, -0.09);
  gy87.setGyroOffsets(2.16, -0.02, -0.18 );
...
void loop() {
  ...
  if (ready) {
    ...
    if (loopTime != loopPrev && loopPrev != 0) {
      // gy87.updateFast();
      if (imuOnly) {
        gy87.updateIMU(deltaTime);
      }
      else {
        gy87.update(deltaTime);
      }
    }
    ...
    // error_roll = gy87.getAngleX() * multiplier;
    // error_pitch = gy87.getAngleY() * multiplier;
    // error_yaw = gy87.getAngleZ() * yaw_k_multiplier * multiplier;
