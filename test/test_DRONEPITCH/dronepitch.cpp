...
void loop() {
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    *desiredValue = newDesiredValue;
    ...
    unsat_throttle[0] = current_motor_speed_0 + P_pitch + I_pitch + D_pitch;
    unsat_throttle[1] = current_motor_speed_1 - P_pitch - I_pitch - D_pitch;
    unsat_throttle[2] = current_motor_speed_2 - P_pitch - I_pitch - D_pitch; // Motor 3 (negative roll, negative pitch, clockwise)
    unsat_throttle[3] = current_motor_speed_3 + P_pitch + I_pitch + D_pitch; // Motor 4 (negative roll, positive pitch, counterclockwise)
