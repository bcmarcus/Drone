...
// int DroneMotor::currMotor = -1;
// uint16_t DroneMotor::motorsTicks[nbMotors] = {0, 0, 0, 0};

void DroneMotor::Init() {
    // Set motors pin PD4, PD5, PD6, PD7 as outputs
    analogWriteFrequency(ESC_PIN_1, 24000);
    analogWriteFrequency(ESC_PIN_2, 24000);
    analogWriteFrequency(ESC_PIN_3, 24000);
    analogWriteFrequency(ESC_PIN_4, 24000);
...
//     DroneMotor::ApplySpeed(TCNTn, OCRnA);
// }
...
// void DroneMotor::Idle() {
//    for (int id = 0; id < nbMotors; id++)
//       motorsTicks[id] = usToTicks(MIN_POWER);
// }

void DroneMotor::UpdateSpeed(int _id, float _PWM) {
   if (_PWM < MIN_POWER) {
...
// void DroneMotor::ApplySpeed(volatile uint16_t *TCNTn, volatile uint16_t *OCRnA) {
